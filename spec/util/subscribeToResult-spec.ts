...
describe('subscribeToResult', () => {
  it('should synchronously complete when subscribed to scalarObservable', () => {
    const result = of(42);
    let expected: number;
    const subscriber = new OuterSubscriber<number, number>(new SafeSubscriber((x) => expected = x));
  ...
  it('should subscribe to observables that are an instanceof Observable', (done) => {
    ...
    const subscriber = new OuterSubscriber<number, number>(new SafeSubscriber(x => {
  ...
  it('should emit error when observable emits error', (done) => {
    ...
    const subscriber = new OuterSubscriber(new SafeSubscriber(x => {
      done(new Error('should not be called'));
    }, (err) => {
  ...
  it('should subscribe to an array and emit synchronously', () => {
    ...
    const subscriber = new OuterSubscriber<number, number>(new SafeSubscriber(x => expected.push(x)));
  ...
  it('should subscribe an iterable and emit results synchronously', () => {
    ...
    const subscriber = new OuterSubscriber(new SafeSubscriber((x: number) => expected = x));
  ...
  it('should pass along errors from an iterable', () => {
    ...
    const subscriber = new OuterSubscriber(new SafeSubscriber({
