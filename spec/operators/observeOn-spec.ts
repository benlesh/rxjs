    ...
    const subscription: any = new Observable<number>(observer => {
      ...
      .subscribe(
        x => {
          ...
          expect(subscription._children.length).to.equal(2);
          const actionSubscription = subscription._children[1];
        ...
        () => {
          ...
          expect(subscription._children.length).to.equal(1);
          const actionSubscription = subscription._children[0];
          expect(actionSubscription.state.notification.kind).to.equal('C');
          // After completion, the entire _children list is nulled out anyhow, so we can't test much further than this.
